...
`;
      ...
      try {
        ...
        for (const item of items) {
          if (item["@type"] === "Product") {
            ...
            if (cat.includes("ink") || name.includes(" ink"))
              return true;
          }
        }
      } catch (_) {
  ...
  function init() {
    ...
    if (!inkInfo) {
      console.warn("[InkScout] Could not extract ink info from Vanness page");
      return;
    }
    const anchor = findAnchorElement();
    if (!anchor) {
      console.warn("[InkScout] Could not find anchor on Vanness page");
      return;
    }
