...
`;
  ...
  // src/content/jetpens.js
  pd;
  function isInkPage() {
    const breadcrumb = document.querySelector('.breadcrumb, .bc-list, [class*="breadcrumb"]');
    if (breadcrumb) {
      const text = breadcrumb.textContent.toLowerCase();
      if (text.includes("ink"))
        return true;
    }
    const title = document.title.toLowerCase();
    return title.includes("ink") || title.includes(" ink ");
  }
  function extractInkInfo() {
    const jsonld = extractFromJsonLD();
    if (jsonld)
      return jsonld;
    const h1 = document.querySelector('h1.product-title, h1[class*="title"], h1[itemprop="name"], h1');
    if (h1) {
      const raw = h1.textContent.trim();
      return parseJetPensTitle(raw);
    }
    const og = document.querySelector('meta[property="og:title"]');
    if (og) {
      return parseJetPensTitle(og.getAttribute("content") || "");
    }
    return null;
  }
  function parseJetPensTitle(title) {
    const brandMeta = document.querySelector('meta[name="brand"], meta[itemprop="brand"]');
    const brand = brandMeta ? (brandMeta.getAttribute("content") || "").trim() : "";
    const inkName = cleanInkName(title, brand);
    return { brand, inkName };
  }
  ...
  function init() {
    ...
    if (!inkInfo) {
      console.warn("[InkScout] Could not extract ink info from JetPens page");
      return;
    }
    const anchor = findAnchorElement();
    if (!anchor) {
      console.warn("[InkScout] Could not find anchor on JetPens page");
      return;
    }
