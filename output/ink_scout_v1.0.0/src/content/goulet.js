...
`;
      ...
      try {
        ...
        for (const item of items) {
          if (item["@type"] === "Product") {
            ...
            const type = (item.category || item.productType || "").toLowerCase();
            if (name.includes("ink") || type.includes("ink"))
              return true;
          }
        }
      } catch (_) {
  ...
  function extractInkInfo() {
    ...
    if (h1) {
      ...
      return parseTitleHeuristic(raw);
    }
    return null;
  }
  function parseTitleHeuristic(title) {
    const brands = [
      "Noodler's",
      "Pilot",
      "Sailor",
    ...
    return { brand: "", inkName: cleanInkName(title, "") };
  }
  ...
  function init() {
    ...
    if (!inkInfo) {
      console.warn("[InkScout] Could not extract ink info from Goulet page");
      return;
    }
    const anchor = findAnchorElement();
    if (!anchor) {
      console.warn("[InkScout] Could not find anchor element on Goulet page");
      return;
    }
